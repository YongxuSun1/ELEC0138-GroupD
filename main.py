...
def check_ip():
    ...
    if 'username' in session:
        ...
        # Retrieve the IP address stored in the session
        session_ip = session.get('ip')
        # If the two IP addresses do not match, it is considered an unauthorized request
        if session_ip and current_ip != session_ip:
            # Clear the session and redirect to the login page
            session.clear()
            flash('Your IP address has changed. Please log in again.', 'error')
            return redirect(url_for('login'))
...
def admin_login():
    if request.method == 'POST':
        ...
        if user and user['password'] == password:
            ...
            return redirect(url_for('admin_login'))
    return render_template('admin_login.html')  # Pass captcha image path to template 
